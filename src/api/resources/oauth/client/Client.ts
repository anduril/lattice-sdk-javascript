...
export class OauthClient {
     ...
     */
    public getToken(request: Lattice.GetTokenRequest, requestOptions?: OauthClient.RequestOptions): core.HttpResponsePromise<Lattice.GetTokenResponse> {
        return core.HttpResponsePromise.fromPromise(this.__getToken(request, requestOptions));
    }

    private async __getToken(request: Lattice.GetTokenRequest, requestOptions?: OauthClient.RequestOptions): Promise<core.WithRawResponse<Lattice.GetTokenResponse>> {
        let _headers: core.Fetcher.Args["headers"] = mergeHeaders(this._options?.headers, requestOptions?.headers);
        const _response = await core.fetcher({
            url: core.url.join(await core.Supplier.get(this._options.baseUrl) ?? (await core.Supplier.get(this._options.environment) ?? environments.LatticeEnvironment.Default), "api/v1/oauth/token"),
            method: "POST",
            headers: _headers,
